...
public class StatisticsService {
	...
	public long calculateScore(int passingAttempts, int passingCompletions, int passingYards, int passingTouchdowns,
			int passingInterceptions, int rushingAttempts, int rushingYards, int rushingTouchdowns,
...
//		//Checks to see if PlayerID is null.  If null create new player with statistics
		if (stats.getPlayer() != null && stats.getPlayer().getPlayerId() == null) {
			return playerService.insertPlayerWithStatistics(stats.getPlayer().getPlayerFirstName(),
					stats.getPlayer().getPlayerLastName(), stats.getTeam(), stats.getPlayer().getPosition(),
					stats.getYear(), stats.getWeekNumber(), stats.getPassingCompletions(), stats.getPassingAttempts(),
					stats.getPassingYards(), stats.getPassingTouchdowns(), stats.getPassingInterceptions(),
					stats.getRushingAttempts(), stats.getRushingYards(), stats.getRushingTouchdowns(),
					stats.getReceivingTargets(), stats.getReceivingReceptions(), stats.getReceivingYards(),
					stats.getReceivingTouchdowns());
		}

		else if ((stats.getPlayer().getPlayerId() != null) && (stats.getPlayer().getPlayerFirstName() == null)
				&& (stats.getPlayer().getPlayerLastName() == null) && (stats.getPlayer().getPosition() == null)) {
			...
			return playerService.insertPlayerWithStatistics(found.get().getPlayerFirstName(),
					...
					stats.getWeekNumber(), stats.getPassingCompletions(), stats.getPassingAttempts(),
					stats.getPassingYards(), stats.getPassingTouchdowns(), stats.getPassingInterceptions(),
					stats.getRushingAttempts(), stats.getRushingYards(), stats.getRushingTouchdowns(),
					stats.getReceivingTargets(), stats.getReceivingReceptions(), stats.getReceivingYards(),
					stats.getReceivingTouchdowns());
		...
		if (stats.getPlayer().getPlayerFirstName().equals(postPlayer.getPlayerFirstName())
			...
			throw new StatisticsException(stats.getPlayer().getPlayerId() + " is not the playerId of"
					+ stats.getPlayer().getPlayerFirstName() + " " + stats.getPlayer().getPlayerLastName()
					+ " or the position is incorect.  Either Leave playerID blank,"
